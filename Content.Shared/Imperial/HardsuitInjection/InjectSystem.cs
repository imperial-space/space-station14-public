...
public sealed class InjectSystem : EntitySystem
{
    ...
    private void UpdateThing(EntityUid uid, InjectComponent component, UpdateEvent args)
    {
        ...
        if (!_solutions.TryGetSolution((ui, Comp<SolutionContainerManagerComponent>(ui)), "chemicals", out var solu, out var solution))
            return;
        var removedSolution = _solutions.SplitSolution((ui, solu.Value.Comp), args.Realtransfer.Value);
        args.End = removedSolution;
        TryComp<AppearanceComponent>(uid, out var appearance);
        _solutions.UpdateAppearance((uid, solu.Value.Comp, appearance));
    }
    ...
    private void Inject(EntityUid uid, InjectComponent component, InjectionEvent args)
    {
        ...
        if (_actionsSystem.TryGetActionData(component.InjectActionEntity, out var action))
        {
            ...
            var realTransferAmount = FixedPoint2.Min(solution.Value.Comp.Solution.Volume, targetSolution.AvailableVolume);
            if (realTransferAmount <= 0)
            {
                _popupSystem.PopupEntity(Loc.GetString("hardsuitinjection-full"), user, user);
                return;
            }
            var ev = new UpdateEvent(GetNetEntity(actualbeaker), solution.Value.Comp.Solution, realTransferAmount);
